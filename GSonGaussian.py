...
def gradient_ascent(noise,x, y, z, mu_x, mu_y, mu_z, sigma_x, sigma_y, sigma_z,SNR=1, learning_rate=0.01, max_iter=1000, step_size = 0.125,aim_value=0.98,stop_steps=0.001):
  if noise==0:
...
def is_success(trajectory, aimpoint):
    ...
    for j in range(covstarting,len(trajectory)):
        countrest+=1
        distance = np.sqrt((trajectory[j][0]-aimpoint[0])*(trajectory[j][0]-aimpoint[0])+(trajectory[j][1]-aimpoint[1])*(trajectory[j][1]-aimpoint[1])+(trajectory[j][2]-aimpoint[2])*(trajectory[j][2]-aimpoint[2]))
        if distance<0.225:
            countsuc+=1
...
def gradient_draw(cmd):
    if cmd == 1 or cmd ==2:
        ...
        for i in [0.5,1,5,10,50,100,300,400,500,600,700,800,1000]:
            stotal_count = 0
            #itotal_count = 0
            success_count = 0
            iterat_count = 0
